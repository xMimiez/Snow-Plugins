    ...
    const r = {
        ...
        async send(channelId, content) {
            ...
            if (typeof util?.sendMessage === 'function') {
                try { util.sendMessage(channelId, body); return true; } catch {}
                try { util.sendMessage(channelId, body, true); return true; } catch {}
                try { util.sendMessage(channelId, text); return true; } catch {}
            }
            return false;
        },
        local(channelId, content) {
            ...
            if (typeof util?.sendBotMessage === 'function') {
                try { util.sendBotMessage(channelId, content); return true; } catch {}
            }
        ...
        command(command) {
            ...
            if (r._nextCommandId == null) r._nextCommandId = -910000 - Math.abs(hashId(meta.id)) % 9000;
            ...
            const remove = register(prepared);
        ...
        hook(names, transform) {
            ...
            if (jsx?.onJsxCreate && jsx?.deleteJsxCreate) {
                for (const name of names) {
                    const callback = (_Component, element) => {
                        ...
                        try { return transform(element, name); } catch (error) { r.error(`JSX ${name}`, error); }
                    };
            ...
            r.patch('after', React, 'createElement', (args, result) => {
                ...
                r.status[name] = (r.status[name] || 0) + 1;
                try { return transform(result, name) ?? result; } catch (error) { r.error(`createElement ${name}`, error); }
            });
        },
        patchRows(transform) {
            const apply = value => {
        ...
        hideSheets() {
            for (const close of [...openSheets.values()]) { try { close(); } catch {} }
            openSheets.clear();
        },
