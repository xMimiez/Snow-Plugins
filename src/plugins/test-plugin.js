...
export default function TestPlugin(r) {
    const { h, React, RN, C } = r, { Page, Text, Button } = ui(r); let report = ['Native Snow API v1', 'Bundled sibling module: 2 + 3 = ' + add(2,3)], busy = false;
    async function run() {
    ...
    function alert() { if (!C.AlertModal || !C.AlertActions || !C.AlertActionButton) return r.toast('Native alert components unavailable'); r.api.ui.openAlert('test', h(C.AlertModal, { title: 'Snow alert test', content: 'This alert is owned by TestPlugin and is dismissed when the plugin stops.', actions: h(C.AlertActions, null, h(C.AlertActionButton, { text: 'Close', onPress: () => r.api.ui.dismissAlert('test') })) })); }
    ...
    return { start() { r.command({ name: 'testplugin', description: 'Open native Snow compatibility checks', execute() { r.open('diagnostics', Settings); } }); }, Settings, run };
}
