var plugin = (() => {
  ...
  var SpotifyPreview_entry_exports = {};
  __export(SpotifyPreview_entry_exports, {
    default: () => SpotifyPreview_entry_default
  });
  ...
  // project:src/plugins/spotify-preview.js
  ...
  function SpotifyPreview(r) {
    ...
    function Preview({ link, fallback, close: dismiss }) {
      ...
      React.useEffect(() => {
        let alive = true;
        r.request(`https://open.spotify.com/oembed?url=${encodeURIComponent(link.url)}`, {}, 8e3, 2e5).then((res) => {
          if (alive) setMeta(res.json());
        }).catch(() => {
        });
        return () => {
          alive = false;
      ...
      React.useEffect(() => {
        const timer = setTimeout(() => setState((s) => s === "loading" ? "ready" : s), 12e3);
      ...
      return h(
        Page,
        { title: meta?.title || "Spotify preview", close: dismiss },
        ...
        h(Button, { text: "Open in Spotify", onPress: async () => {
          try {
        ...
        fallback ? h(Button, { text: "Open original link", variant: "secondary", onPress: () => {
          dismiss();
          fallback();
        } }) : null,
        h(Button, { text: "Retry", variant: "secondary", onPress: () => {
          setState("loading");
          retry((n) => n + 1);
        } })
      );
    ...
    function show(link) {
      try {
        close?.();
        close = r.open("preview", Preview, { link, fallback: null });
        return true;
      } catch (error) {
        try {
          r.api.ui.openAlert("spotify-preview", r.h(r.D.AlertModal || r.C.AlertModal, {
            ...
            actions: r.h(
              r.D.AlertActions || r.C.AlertActions,
              null,
      ...
      start() {
        ...
        for (const key of Object.keys(modules)) {
          if (typeof modules[key]?.openURL === "function") {
            try {
              r.patch("instead", modules[key], "openURL", intercept);
            } catch {
            }
          }
        }
  ...
  // SpotifyPreview.entry.js
  var SpotifyPreview_entry_default = register({ "id": "mime.spotifypreview", "name": "SpotifyPreview", "description": "Official Spotify embeds in an owned Snow sheet.", "version": "2.2.2", "authors": [{ "name": "Mime | N0_.q3", "id": "957164619061932045" }], "license": "MIT", "source": "https://github.com/xMimiez/Snow-Plugins/tree/main/SpotifyPreview" }, SpotifyPreview);
