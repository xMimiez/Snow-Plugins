(() => {
  ...
  function SpotifyPreview(r) {
    ...
    function Preview({ link, fallback, close: dismiss }) {
      ...
      React.useEffect(() => {
        const timer = setTimeout(() => setState((s) => s === "loading" ? "error" : s), 2e4);
      ...
      return h(
        ...
        h(WebView, {
          ...
          onShouldStartLoadWithRequest: (request) => {
            try {
              ...
              return u.protocol === "https:" && (u.hostname === "open.spotify.com" || u.hostname.endsWith(".spotify.com") || u.hostname === "spotify.com");
            } catch {
              return false;
            }
          }
        ...
        state === "error" ? h(Button, { text: "Retry", variant: "secondary", onPress: () => {
          setState("loading");
          retry((n) => n + 1);
        } }) : null
      );
    ...
    function Settings() {
      r.useRefresh();
      return h(Page, { title: "SpotifyPreview" }, h(Toggle, { setting: "enabled", label: "Preview Spotify links" }), h(Text, { muted: true }, "Uses the official Spotify embed. Links still open normally if Snow\u2019s sheet or WebView is unavailable."));
    }
    return { start() {
  ...
  register({ "id": "mime.spotifypreview", "name": "SpotifyPreview", "description": "Official Spotify embeds in an owned Snow sheet.", "version": "2.0.0", "authors": [{ "name": "Mime | N0_.q3", "id": "957164619061932045" }], "license": "MIT", "source": "https://github.com/xMimiez/Snow-Plugins/tree/main/SpotifyPreview", "capabilities": ["storage", "ui"], "dependencies": [] }, SpotifyPreview);
})();
