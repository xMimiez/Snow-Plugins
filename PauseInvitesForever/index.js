var plugin = (() => {
  ...
  var PauseInvitesForever_entry_exports = {};
  __export(PauseInvitesForever_entry_exports, {
    default: () => PauseInvitesForever_entry_default
  });
  ...
  function PauseInvitesForever(r) {
    ...
    function Confirm({ guildId, pause, close }) {
      ...
      return h(
        Page,
        { title: pause ? "Pause server invites indefinitely?" : "Resume server invites?", close },
        h(Text, null, `Server: ${r.byStore("GuildStore")?.getGuild?.(guildId)?.name || guildId}. This changes the invite setting for everyone and requires server permissions.`),
        h(Button, { text: busy ? "Saving\u2026" : pause ? "Pause invites" : "Resume invites", disabled: busy, onPress: async () => {
          ...
          try {
            ...
            r.toast(pause ? "Invites paused" : "Invites resumed");
            if (mounted.current) close();
          } catch (e) {
            if (r.active) r.error("Server invites", e);
          } finally {
            inFlight.current = false;
            if (r.active && mounted.current) setBusy(false);
          }
        } })
      );
    }
    return { start() {
      for (const pause of [true, false]) r.command({ name: pause ? "pauseinvites" : "resumeinvites", description: pause ? "Pause server invites without a timer" : "Resume server invites", execute(_options, context) {
        ...
        r.open("confirm", Confirm, { guildId, pause });
      } });
    }, Settings() {
      return h(Page, { title: "PauseInvitesForever" }, h(Text, null, "Use /pauseinvites or /resumeinvites in a server. A confirmation appears before changing anything. Current server features are preserved; Discord validates your permissions."));
    }, Confirm };
  }

  // PauseInvitesForever.entry.js
  var PauseInvitesForever_entry_default = register({ "id": "mime.pauseinvitesforever", "name": "PauseInvitesForever", "description": "Pause and resume server invites with explicit confirmation.", "version": "1.1.0", "authors": [{ "name": "Dolfies", "id": "852892297661906993" }, { "name": "amia", "id": "142007603549962240" }, { "name": "Mime | N0_.q3", "id": "957164619061932045" }], "license": "GPL-3.0-or-later", "source": "https://github.com/xMimiez/Snow-Plugins/tree/main/PauseInvitesForever" }, PauseInvitesForever);
