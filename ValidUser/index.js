var plugin = (() => {
  ...
  // ValidUser.entry.js
  var ValidUser_entry_exports = {};
  __export(ValidUser_entry_exports, {
    default: () => ValidUser_entry_default
  });
  ...
  function ValidUser(r) {
    ...
    async function resolve(id) {
      ...
      if (Date.now() < next) throw new Error("Wait a moment before another lookup");
      next = Date.now() + 1500;
      const task = r.discord(`/users/${id}`).then((result) => {
    ...
    function Settings() {
      ...
      return h(Page, { title: "ValidUser" }, h(Input, { label: "User ID", value: id, onChange: setId, keyboardType: "number-pad" }), h(Button, { text: busy ? "Resolving\u2026" : "Resolve user", disabled: busy, onPress: () => {
        setBusy(true);
      ...
      } }), h(Text, null, result || "Mobile lookup populates available user data. Desktop hover and inferred badges are not reproduced."));
    }
    return { start() {
      r.command({ name: "resolveuser", description: "Resolve a Discord user ID locally", options: [{ name: "id", description: "User ID", type: 3, required: true }], async execute(options) {
  ...
  // ValidUser.entry.js
  var ValidUser_entry_default = register({ "id": "mime.validuser", "name": "ValidUser", "description": "Resolve a user ID and populate available user data.", "version": "1.1.0", "authors": [{ "name": "Vendicated", "id": "343383572805058560" }, { "name": "Dolfies", "id": "852892297661906993" }, { "name": "Mime | N0_.q3", "id": "957164619061932045" }], "license": "GPL-3.0-or-later", "source": "https://github.com/xMimiez/Snow-Plugins/tree/main/ValidUser" }, ValidUser);
  return __toCommonJS(ValidUser_entry_exports);
})();
