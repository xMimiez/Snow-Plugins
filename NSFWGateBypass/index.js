var plugin = (() => {
  ...
  // NSFWGateBypass.entry.js
  var NSFWGateBypass_entry_exports = {};
  __export(NSFWGateBypass_entry_exports, {
    default: () => NSFWGateBypass_entry_default
  });
  ...
  function NsfwGateBypass(r) {
    const { h } = r, { Page, Text } = ui(r);
    return { start() {
      let count = 0;
      for (const name of ["isNSFWInvite", "shouldNSFWGateGuild"]) {
        if (r.patch("after", r.find(name), name, () => false)) count++;
      }
      r.status.support = count ? `${count} client gate hooks installed` : "Unsupported: mobile gate functions were not found";
    }, Settings() {
      return h(Page, { title: "NSFWGateBypass" }, h(Text, null, r.status.support), h(Text, { muted: true }, "Removes supported local guild interstitials. Server permissions and account restrictions remain enforced."));
    } };
  }
  ...
  // NSFWGateBypass.entry.js
  var NSFWGateBypass_entry_default = register({ "id": "mime.nsfwgatebypass", "name": "NSFWGateBypass", "description": "Bypass supported local guild interstitials without mutating account settings.", "version": "1.1.0", "authors": [{ "name": "eternal", "id": "263689920210534400" }, { "name": "Mime | N0_.q3", "id": "957164619061932045" }], "license": "GPL-3.0-or-later", "source": "https://github.com/xMimiez/Snow-Plugins/tree/main/NSFWGateBypass" }, NsfwGateBypass);
  return __toCommonJS(NSFWGateBypass_entry_exports);
})();
