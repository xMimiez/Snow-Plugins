var plugin = (() => {
    ...
    return {
      start() {
        ...
        r.patch("after", r.byStore("UserStore") || r.find("getCurrentUser"), "getCurrentUser", (_args, user) => {
          if (user && user.nsfwAllowed === false) try {
      ...
      Settings() {
        return h(Page, { title: "NSFWGateBypass" }, h(Text, null, r.status.support), h(Text, { muted: true }, "Local interstitial only. Server permissions stay enforced."));
      }
    };
  }
  ...
  // NSFWGateBypass.entry.js
  var NSFWGateBypass_entry_default = register({ "id": "mime.nsfwgatebypass", "name": "NSFWGateBypass", "description": "Bypass supported local guild interstitials without mutating account settings.", "version": "1.2.0", "authors": [{ "name": "eternal", "id": "263689920210534400" }, { "name": "Mime | N0_.q3", "id": "957164619061932045" }], "license": "GPL-3.0-or-later", "source": "https://github.com/xMimiez/Snow-Plugins/tree/main/NSFWGateBypass" }, NsfwGateBypass);
