var plugin = (() => {
  ...
  function ReplyToStatus(r) {
    ...
    function Emoji({ part }) {
      ...
      return h(RN.Image, {
        source: { uri: `https://cdn.discordapp.com/emojis/${part.id}.${part.animated ? "gif" : "png"}?size=48` },
...
` : "";
    ...
    function open(userId, status) {
      if (dismiss) return;
      try {
        dismiss = r.open("composer", Composer, { userId, status: status || { text: "" } });
      } catch (error) {
        dismiss = null;
    ...
    function overlay(tree, props) {
      ...
      return h(
        ...
        h(
          RN.View,
          { style: { position: "absolute", top: 6, right: 44, zIndex: 80 } },
          IconButton ? h(IconButton, { icon: "ChatIcon", label: "Reply to Status", size: "sm", variant: "secondary", onPress: () => open(userId, status) }) : h(Button, { text: "Reply", variant: "secondary", onPress: () => open(userId, status) })
        )
      );
    }
    ...
    return {
      start() {
        ...
        r.hook(names, (element) => {
          ...
          return tree === element ? void 0 : tree;
        });
      },
      stop() {
        dismiss?.();
  ...
  // ReplyToStatus.entry.js
  var ReplyToStatus_entry_default = register({ "id": "mime.replytostatus", "name": "ReplyToStatus", "description": "Reply button on the top-right of the profile card.", "version": "2.2.7", "authors": [{ "name": "Mime | N0_.q3", "id": "957164619061932045" }], "license": "MIT", "source": "https://github.com/xMimiez/Snow-Plugins/tree/main/ReplyToStatus" }, ReplyToStatus);
